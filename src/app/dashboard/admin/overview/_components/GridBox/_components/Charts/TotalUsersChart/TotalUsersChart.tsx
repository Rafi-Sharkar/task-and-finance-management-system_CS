...
import { UserStats } from "../../../GridBox"; // Ensure this path is correct
...
export default function TotalUsersChart({ userStats }: UserStats) {
  // 1. Create dynamic chart data from props
  ...
  // 2. Use the total from API or calculate it
          ...
          {Object.entries(chartConfig).map(([key, config]) => (
            <div key={key} className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-[#414651]">
                {config?.label} ({(userStats as Record<string, number>)?.[key] || 0})
              </span>
            </div>
          ))}
        </div>
