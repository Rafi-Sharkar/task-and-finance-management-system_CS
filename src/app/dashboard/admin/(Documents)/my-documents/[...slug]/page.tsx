...
const DynamicDocumentManagementPage = () => {
  ...
  const handleBackNavigation = () => {
    if (!slug || slug.length <= 1) {
      router.push("/dashboard/admin/my-documents");
    } else {
      const parentSlug = slug.slice(0, -1).join("/");
      router.push(`/dashboard/admin/my-documents/${parentSlug}`);
    }
  };

  const handleFolderClick = (folder: FolderType) => {
    const currentPath = slug ? slug.join("/") : "";
    router.push(`/dashboard/admin/my-documents/${currentPath}/${folder.id}`);
  };
