...
function FinanceClientsInvoiceTable() {
  ...
  const { data: invoicesData, isLoading, isFetching } = useGetAllInvoicesQuery({ invoiceStatus:statusFilter, invoiceType: "SELLS", ...query });
      ...
      cell: (row) => (
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end" className="w-36">
            <DropdownMenuItem disabled={row?.invoiceStatus === "PAID"} onClick={() => handleStatusChange(row?.id)} className="cursor-pointer hover:bg-gray-50 disabled:cursor-not-allowed">
              {
                isChangeInvoiceStatusLoading ? "Updating..." : " Mark As Paid"
              }
            </DropdownMenuItem>
            <DropdownMenuItem className="cursor-pointer font-medium text-[#155DFC]">
              View
            </DropdownMenuItem>
          </DropdownMenuContent>
        </DropdownMenu>
...
export default FinanceClientsInvoiceTable;
