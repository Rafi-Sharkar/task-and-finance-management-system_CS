...
import { useGetAuditMyLogsQuery } from "@/redux/features/admin/auditLogs/auditLogs.api";
...
function EmployeeAuditLogsTable() {
  ...
  const handleSearchUsers = (documentName: string) => {
    if (documentName === "") {
      deleteQuery("search");
    } else {
      setQuery("search", documentName);
    }
  };
  ...
  return (
    <div className="w-full space-y-5">
      ...
      {!isLoading && !isFetching && auditLogsResponse?.data?.data?.length > 0 && (
        <>
...
export default EmployeeAuditLogsTable;
